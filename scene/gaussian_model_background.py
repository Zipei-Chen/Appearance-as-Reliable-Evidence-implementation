...
class GaussianModelBackground:
    ...
    def clone(self):
        cloned = GaussianModelBackground(self.cfg)
    ...
    def restore(self, model_args, training_args):
        ...
        # sample_num = 100000
        #
        # indices = torch.randperm(len(self._xyz))[:sample_num]
        # # 采样
    ...
    def get_opacity(self):
        # return self.opacity_activation(self._opacity + inverse_sigmoid(0.1 * torch.ones_like(self._opacity)))
    ...
    def create_from_pcd(self, pcd : BasicPointCloud, spatial_lr_scale=1., verts=None, froze=True):
        self.spatial_lr_scale = spatial_lr_scale
        fused_point_cloud = torch.tensor(np.asarray(pcd.points)).float().cuda()
        # fused_point_cloud = torch.tensor(verts).float().cuda()
        ...
        if froze:
            self._xyz = nn.Parameter(fused_point_cloud.requires_grad_(False), requires_grad=False)
            # self._xyz = nn.Parameter(fused_point_cloud.requires_grad_(False), requires_grad=False)
            self._features_dc = nn.Parameter(features[:,:,0:1].transpose(1, 2).contiguous().requires_grad_(False), requires_grad=False)
            self._features_rest = nn.Parameter(features[:,:,1:].transpose(1, 2).contiguous().requires_grad_(False), requires_grad=False)
            self._scaling = nn.Parameter(scales.requires_grad_(False), requires_grad=False)
            self._rotation = nn.Parameter(rots.requires_grad_(False), requires_grad=False)
            self._opacity = nn.Parameter(opacities.requires_grad_(False), requires_grad=False)
    ...
    def _prune_optimizer(self, mask):
        ...
        for group in self.optimizer.param_groups:
            ...
            if stored_state is not None:
                ...
                # if group["name"] == "xyz":
                #     group["params"][0] = nn.Parameter(group["params"][0][mask].requires_grad_(False), requires_grad=False)
                # else:
                group["params"][0] = nn.Parameter(group["params"][0][mask].requires_grad_(True))
                optimizable_tensors[group["name"]] = group["params"][0]
        return optimizable_tensors
