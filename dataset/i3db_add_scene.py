...
class I3dbAddScene(Dataset):
    def __init__(self, cfg, split='train', is_coarse=False):
        ...
        with open(pjoin(self.root_dir, self.recording_name, "camera_info.pkl"), 'rb') as f:
        ...
        with open(pjoin(self.root_dir, self.recording_name, "gender.txt"), 'r') as file:
            self.gender_gt = file.read()

        if not is_coarse:
            with open(pjoin(self.root_dir, self.recording_name, "pose_info.pkl"), 'rb') as f:
                model_files = pkl.load(f)

        else:
            with open(pjoin(self.root_dir, self.recording_name, "coarse_pose_info.pkl"), 'rb') as f:
                model_files = pkl.load(f)
        ...
        self.data = []
        img_dir = pjoin(self.root_dir, self.recording_name, "raw_frames")
        mask_dir = pjoin(self.root_dir, self.recording_name, "mask_predict_deeplab")
        img_files = [sample for sample in sorted(glob.glob(os.path.join(img_dir, '*.jpg')))]
        mask_files = [sample for sample in sorted(glob.glob(os.path.join(mask_dir, '*.png')))]
    ...
    def get_cano_smpl_verts(self, model_files, gender):
        '''
        ...
        cano_mesh_block = trimesh.Trimesh(vertices=vertices.astype(np.float32), faces=self.faces)
    ...
    def get_smpl_data(self, model_files):
        ...
        for idx, (frame) in enumerate(zip(self.frames)):
            # if idx == 0:
            smpl_data['betas'].append(model_dict['beta'][frame].astype(np.float32))
    ...
    def getitem(self, idx):
        ...
        gender_gt = self.metadata['gender']
        ...
        pose = np.concatenate([root_orient, pose_body], axis=-1)
    ...
    def readPointCloud(self, is_scene=False):
        if self.cfg.get('random_init', False):
            ...
            if is_scene:
                # ply_path = os.path.join(self.root_dir, self.recording_name, "tmp_result_scene", "20000", "deformed_pose_add_color.ply")
                ply_path = os.path.join(self.root_dir, self.recording_name, "tmp_result_scene", "20000", "original_gaussian.ply")
                pcd = fetchPly(ply_path)
            else:
                ply_path = os.path.join(self.root_dir, self.recording_name, 'cano_smpl.ply')
                # try:
                #     pcd = fetchPly(ply_path)
                # except:
