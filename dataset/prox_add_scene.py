...
class ProxAddScene(Dataset):
    def __init__(self, cfg, split='train', is_coarse=False):
        ...
        if not is_coarse:
            ...
            with open(pjoin(self.root_dir, "recordings", self.recording_name, "coarse_pose_info.pkl"), 'rb') as f:
                model_files = pkl.load(f)
        ...
        # mask_dir = pjoin(self.root_dir, "recordings", self.recording_name, "BodyIndexColor")
    ...
    def get_smpl_data(self):
        ...
        for idx, (frame) in enumerate(zip(self.frames)):
            ...
            # if idx == 0:
            #     smpl_data['betas'].append(model_dict['beta'][0].astype(np.float32))
    ...
    def getitem(self, idx):
        ...
        # else:
        # bone_transforms = model_dict['transform_list'][frame_idx].detach().cpu().numpy()
    ...
    def readPointCloud(self, is_scene=False):
        if self.cfg.get('random_init', False):
            ...
            if is_scene:
                ply_path = os.path.join(self.root_dir, "recordings", self.recording_name, "tmp_result_scene", "20000", "deformed_pose_add_color.ply")
                pcd = fetchPly(ply_path)
            else:
