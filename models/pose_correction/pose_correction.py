...
def get_transforms_02v(Jtr):
    ...
    bone_transforms_02v = torch.eye(4, dtype=torch.float32, device=device).reshape(1, 4, 4).repeat(55, 1, 1)
...
class PoseCorrection(nn.Module):
    def __init__(self, config, metadata=None):
        ...
        # self.metadata_unblock = metadata_unblock

        self.frame_dict = metadata['frame_dict']

        gender = metadata['gender']

        self.smplx_neutral = smplx.create(model_path="./body_models/smplx_model", model_type="smplx", gender=gender,
        ...
        v_template = np.load('body_models/misc_smplx/v_templates.npz')[gender]
        lbs_weights = np.load('body_models/misc_smplx/skinning_weights_all.npz')[gender]
        posedirs = np.load('body_models/misc_smplx/posedirs_all.npz')[gender]
        posedirs = posedirs.reshape([posedirs.shape[0] * 3, -1]).T
        shapedirs = np.load('body_models/misc_smplx/shapedirs_all.npz')[gender]
        J_regressor = np.load('body_models/misc_smplx/J_regressors.npz')[gender]
        kintree_table = np.load('body_models/misc_smplx/kintree_table.npy')
        ...
        # else:
        #     gender_block = metadata['gender']
        #     gender_unblock = metadata['gender']
        #
        #     # v_template = np.load('body_models/misc/v_templates.npz')[gender]
        #     # lbs_weights = np.load('body_models/misc/skinning_weights_all.npz')[gender]
        #     # posedirs = np.load('body_models/misc/posedirs_all.npz')[gender]
        ...
        #     self.register_buffer('kintree_table', torch.tensor(kintree_table, dtype=torch.int32))
        #
        #     v_template = np.load('body_models/misc_smplx/v_templates.npz')[gender_block]
        #     lbs_weights = np.load('body_models/misc_smplx/skinning_weights_all.npz')[gender_block]
        #     posedirs = np.load('body_models/misc_smplx/posedirs_all.npz')[gender_block]
        #     posedirs = posedirs.reshape([posedirs.shape[0] * 3, -1]).T
        #     shapedirs = np.load('body_models/misc_smplx/shapedirs_all.npz')[gender_block]
        #     J_regressor = np.load('body_models/misc_smplx/J_regressors.npz')[gender_block]
        #
        #     self.register_buffer('v_template_block', torch.tensor(v_template, dtype=torch.float32).unsqueeze(0))
        #     self.register_buffer('posedirs_block', torch.tensor(posedirs, dtype=torch.float32))
        #     self.register_buffer('shapedirs_block', torch.tensor(shapedirs, dtype=torch.float32))
        #     self.register_buffer('J_regressor_block', torch.tensor(J_regressor, dtype=torch.float32))
        #     self.register_buffer('lbs_weights_block', torch.tensor(lbs_weights, dtype=torch.float32))
        #
        #     v_template = np.load('body_models/misc_smplx/v_templates.npz')[gender_unblock]
        #     lbs_weights = np.load('body_models/misc_smplx/skinning_weights_all.npz')[gender_unblock]
        #     posedirs = np.load('body_models/misc_smplx/posedirs_all.npz')[gender_unblock]
        #     posedirs = posedirs.reshape([posedirs.shape[0] * 3, -1]).T
        #     shapedirs = np.load('body_models/misc_smplx/shapedirs_all.npz')[gender_unblock]
        #     J_regressor = np.load('body_models/misc_smplx/J_regressors.npz')[gender_unblock]
        #
        #     self.register_buffer('v_template_unblock', torch.tensor(v_template, dtype=torch.float32).unsqueeze(0))
        #     self.register_buffer('posedirs_unblock', torch.tensor(posedirs, dtype=torch.float32))
        #     self.register_buffer('shapedirs_unblock', torch.tensor(shapedirs, dtype=torch.float32))
        #     self.register_buffer('J_regressor_unblock', torch.tensor(J_regressor, dtype=torch.float32))
        #     self.register_buffer('lbs_weights_unblock', torch.tensor(lbs_weights, dtype=torch.float32))
    ...
    def forward_smpl(self, betas, root_orient, pose_body, pose_hand, trans):
        ...
        # bone_transforms_02v = get_transforms_02v(Jtrs.squeeze(0))[:22, :, :]
    ...
    def forward(self, camera, iteration, is_block=False, is_unblock=False):
        frame = camera.frame_id
        if frame not in self.frame_dict:
            return camera, {}
        return self.pose_correct(camera, iteration, is_block, is_unblock)
...
class DirectPoseOptimization(PoseCorrection):
    def __init__(self, config, metadata=None):
        ...
        pose_body = np.concatenate([pose_body, np.zeros([pose_body.shape[0], 99], dtype=np.float32)], axis=-1)
        ...
        self.betas = nn.Embedding.from_pretrained(torch.from_numpy(betas).float(), freeze=False)
        ...
        #     frames = metadata['frames']
        #
        #     root_orient_block = metadata['root_orient_block']
        #     pose_body_block = metadata['pose_body_block']
        #     trans_block = metadata['trans_block']
        #     betas_block = metadata['betas_block']
        #
        #     root_orient_unblock = metadata['root_orient_unblock']
        #     pose_body_unblock = metadata['pose_body_unblock']
        #     trans_unblock = metadata['trans_unblock']
        #     betas_unblock = metadata['betas_unblock']
        #
        #     self.frames = frames
        #
        #     # use nn.Embedding
        ...
        #     self.pose_hands = nn.Embedding.from_pretrained(torch.from_numpy(pose_hand).float(), freeze=False)
        #
        #     root_orient = np.array(root_orient_block)
        #     pose_body = np.array(pose_body_block)
        #     pose_body = np.concatenate([pose_body, np.zeros([pose_body.shape[0], 99], dtype=np.float32)], axis=-1)
        #     trans = np.array(trans_block)
        #
        #     self.root_orients_block = nn.Embedding.from_pretrained(torch.from_numpy(root_orient).float(), freeze=False)
        #     self.pose_bodys_block = nn.Embedding.from_pretrained(torch.from_numpy(pose_body).float(), freeze=False)
        #     self.trans_block = nn.Embedding.from_pretrained(torch.from_numpy(trans).float(), freeze=False)
        #     self.register_parameter('betas_block', nn.Parameter(torch.tensor(betas_block, dtype=torch.float32)))
        #
        #     root_orient = np.array(root_orient_unblock)
        #     pose_body = np.array(pose_body_unblock)
        #     pose_body = np.concatenate([pose_body, np.zeros([pose_body.shape[0], 99], dtype=np.float32)], axis=-1)
        #     trans = np.array(trans_unblock)
        #
        #     self.root_orients_unblock = nn.Embedding.from_pretrained(torch.from_numpy(root_orient).float(), freeze=False)
        #     self.pose_bodys_unblock = nn.Embedding.from_pretrained(torch.from_numpy(pose_body).float(), freeze=False)
        #     self.trans_unblock = nn.Embedding.from_pretrained(torch.from_numpy(trans).float(), freeze=False)
        #     self.register_parameter('betas_unblock', nn.Parameter(torch.tensor(betas_unblock, dtype=torch.float32)))

    def pose_correct(self, camera, iteration, is_block=False, is_unblock=False):
        ...
        # use nn.Embedding
        idx = torch.Tensor([self.frame_dict[frame]]).long().cuda()
        ...
        new_gaussian_xyz = self.smplx_neutral.forward(betas=betas).vertices

        if is_block == True:
            rots_diff = camera.rots_block[:, :22, :] - rots[:, :22, :]
            # rots_diff = camera.rots_block[:, :24, :] - rots[:, :24, :]
            updated_camera = camera.copy()
            updated_camera.update(
                rots_block=rots,
                # Jtrs=Jtrs,
                bone_transforms_block=bone_transforms,
            )
        elif is_unblock == True:
            rots_diff = camera.rots_unblock[:, :22, :] - rots[:, :22, :]
            # rots_diff = camera.rots_unblock[:, :24, :] - rots[:, :24, :]
            updated_camera = camera.copy()
            updated_camera.update(
                rots_unblock=rots,
                # Jtrs=Jtrs,
                bone_transforms_unblock=bone_transforms,
            )
        else:
            rots_diff = camera.rots[:, :22, :] - rots[:, :22, :]
            # rots_diff = camera.rots[:, :24, :] - rots[:, :24, :]
            updated_camera = camera.copy()
            updated_camera.update(
                rots=rots,
                # Jtrs=Jtrs,
