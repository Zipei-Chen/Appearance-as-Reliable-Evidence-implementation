...
def lbs(betas, pose, v_template, clothed_v_template, shapedirs, posedirs, J_regressor, parents,
    ...
    v_shaped = v_template + blend_shapes(betas, shapedirs[..., :len(betas[0])])
    ...
    rot_mats = rot_mats[:, :55, :, :]
    J = J[:, :55, :]
    parents = parents[:55]
    J_regressor = J_regressor[:55, :]

    # rot_mats = rot_mats[:, :24, :, :]
    # J = J[:, :24, :]
    # parents = parents[:24]
    # J_regressor = J_regressor[:24, :]
