...
class ColorMLP(ColorPrecompute):
    def __init__(self, cfg, metadata, is_scene=False):
        super().__init__(cfg, metadata)
    ...
    def compose_input(self, gaussians, camera, is_scene=False):
        ...
        if self.sh_degree > 0:
            ...
            if self.cano_view_dir and not is_scene:
                T_fwd = gaussians.fwd_transform
                R_bwd = T_fwd[:, :3, :3].transpose(1, 2)
                dir_pp = torch.matmul(R_bwd, dir_pp.unsqueeze(-1)).squeeze(-1)
                view_noise_scale = self.cfg.get('view_noise', 0.)
    ...
    def forward(self, gaussians, camera, is_scene=False):
        inp = self.compose_input(gaussians, camera, is_scene)
        output = self.mlp(inp)
        color = self.color_activation(output)
        return color


def get_texture(cfg, metadata, is_scene=False):
    name = cfg.name
    model_dict = {
        "sh2rgb": SH2RGB,
        "mlp": ColorMLP,
    }
    return model_dict[name](cfg, metadata, is_scene)