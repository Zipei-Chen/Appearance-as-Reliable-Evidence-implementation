...
class Identity(RigidDeform):
    ...
    def forward(self, gaussians, iteration, camera, is_block=False, is_unblock=False):
        return gaussians
...
def hierarchical_softmax_modify(x):
    def softmax(x):
        return F.softmax(x, dim=-1)
    ...
    prob_all = torch.ones(n_point, 22, device=x.device)
    # softmax_x = F.softmax(x, dim=-1)
    sigmoid_x = sigmoid(x).float()
    ...
    prob_all[:, [12, 13, 14]] = prob_all[:, [9]] * sigmoid_x[:, [22]] * softmax(x[:, [12, 13, 14]])
    prob_all[:, [9]] = prob_all[:, [9]] * (1 - sigmoid_x[:, [22]])
...
class SkinningField(RigidDeform):
    ...
    def get_forward_transform(self, xyz, tfs):
        if self.distill:
            ...
            # pts_W = torch.from_numpy(self.skinning_weights[:, :55]).cuda()
    ...
    def sample_skinning_loss(self):
        ...
        pts_W = (self.skinning_weights[vert_ids][:, :, :55] * bary_coords[..., None]).sum(axis=1)
        # pts_W = (self.skinning_weights[vert_ids][:, :, :24] * bary_coords[..., None]).sum(axis=1)
        # pts_W = ((F.softmax(torch.from_numpy(self.skinning_weights[vert_ids][:, :, :22]), dim=-1)).numpy() * bary_coords[..., None]).sum(axis=1)
    ...
    def softmax(self, logit):
        ...
        elif logit.shape[-1] == 23:
            w = hierarchical_softmax_modify(logit)
    ...
    def forward(self, gaussians, iteration, camera, is_block=False, is_unblock=False):
        if is_block == True:
            tfs = camera.bone_transforms_block
        elif is_unblock == True:
            tfs = camera.bone_transforms_unblock
        else:
        ...
        # rgb = np.ones_like(x_bar.detach().cpu()) * 255
        # storePly("C:/RoHM/datasets/PROX/recordings/N0Sofa_00034_02/deformed_pose.ply", x_bar.detach().cpu(), rgb)
