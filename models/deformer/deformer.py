...
class Deformer(nn.Module):
    ...
    def forward(self, gaussians, camera, iteration, compute_loss=True, is_block=False, is_unblock=False):
        loss_reg = {}
        deformed_gaussians, loss_non_rigid = self.non_rigid(gaussians, iteration, camera, compute_loss)
        deformed_gaussians = self.rigid(deformed_gaussians, iteration, camera, is_block, is_unblock)
